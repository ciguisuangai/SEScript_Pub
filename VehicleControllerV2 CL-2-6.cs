...
//各档位最大车速
static float[] MaxSpeed = {20f, 40f, 70f, 100f, 140f, 180f, 250f, 320f};

const float MaxRevSpeed = 20f;					//最大倒车速度
const float FrontToe = 0f;						//前轮束角（废弃变量）
...
const int RefreshTime = 10;			//刷新间隔（单位：秒）

//转向角曲线设定
double[,] SteerCurve = new double[4,2]{{5, 20}, {15, 6}, {30, 4}, {50, 8}};
...
void ProcessDisplay()
{
	...
	{
		ClearScreen(CockpitSurfaces.GetSurface(0));
	...
	DrawBatteryScreen(CockpitSurfaces.GetSurface(1), Brightness);
	
	if (UnderControlTimer >= 6)
	{
		DrawMainScreen(CockpitSurfaces.GetSurface(0), Brightness);
...
void ProcessMain()
{
	...
	if (Cockpit != null)
	{
		...
		Midair = -VertAccel > GravityLength*0.8 && TerrDist > 4;
